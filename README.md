# Reel — Releases & Update Feed

This repository hosts the [Sparkle](https://sparkle-project.org) auto-update
feed and the release binaries for **Reel**, the macOS screen-recording app.
It is intentionally **public**: Reel has a free tier, so unlicensed and
...
- **Release binaries** — each notarized `Reel-<version>.zip` is attached as a
  **GitHub Release asset** on its version tag, e.g.
  `https://github.com/kylemclaren/reel-releases/releases/download/v1.0.0/Reel-1.0.0.zip`.
...
The feed is **not** edited by hand. In the main (private) `reel` repo,
...
`<reel:releaseDate>` element the app's updater uses to decide whether an
...
Enclosures are signed with Reel's Ed25519 (EdDSA) key; the public half is
